...
def entropy_sensor_selection(bp_base, status_nodes, rho_max, m, max_iter, tol, damp, delta, logger=None, G=None, alpha=0.5, beta=0.3, gamma=0.5):
    ...
    for k in tqdm(range(target + 1)):
        ...
        tensions = tension_metric(marginals, status_nodes, bp_base, epsilon=1e-10) #frontier_score(marginals, G=G, t=0) #adaptive_score(marginals, Mt, G, alpha=alpha, decay=0.3)
        tensions = (tensions - tensions.min()) / (tensions.max() - tensions.min() + 1e-12)
        scores = p_inf * (1 + gamma * tensions)
        # select top_k to inspect
        scores_heuristic = scores  # for comparison, the "base" score without tension
        top_k = 10
        # select by p_inf score
        scores_heuristic = p_inf
        top_k_nodes =  [remaining[i] for i in np.argsort(scores_heuristic[remaining])[-top_k:]]
        #best_candidate = remaining[np.argmax(scores[remaining])]
        saved_messages = torch.clone(bp_base.messages.values)
        remaining = list(set(range(bp_base.size)) - sensor_set)
        #print("Current observations:", len(sensor_set))
...
#select_best_candidate(remaining, marginals, G) #select_best_candidate(candidates = np.random.choice(remaining, size=min(20, len(remaining)), replace=False), G = G, bp_base = bp_base, saved_messages = saved_messages, current_obs = current_obs, delta = delta, tol = tol, damp = damp)
        #best_candidate = max(scores, key=scores.get)
        #print("Obs shape after selection:", current_obs.shape)
        ...
        # update with bp_base.observations (this is a torch tensor, convert to numpy for consistency)
        new_rows = build_obs({best_candidate}, status_nodes)
        ...
        if k < 5 or k % 20 == 0:
            ...
            #print("Selected candidate:", best_candidate, "CMOV:", cmov)
