...
def sequential_sensor_selector(metric, bp_base, status_nodes, rho_max, m, max_iter, tol, damp, delta, collector=None, G=None, lam=None, sim_id=None):
    ...
    k = len(sensor_set)
    while k < target:
        # 1. Start a fresh buffer for this step's candidates
        ...
        # 2. eval candidates
        best_candidate = eval_candidates(metric=metric, collector=collector, metric_base=metric_base, candidates=candidates, bp_base=bp_base, saved_messages=saved_messages, status_nodes=status_nodes, current_obs=current_obs, warm_iter=20, tol=tol, damp=damp, delta=delta, k=k)
        # 3. Commit this step as a training sample
        # Important: Record state BEFORE adding the sensor
        collector.commit_step(bp_base.marginals(), G, delta, lam, sim_id, k, sensor_set)
        ...
        k = len(sensor_set)
        if k < 5 or k % 20 == 0:
            print(f"[Step {k}/{target}] selected sensor {best_candidate}, metric={metric_value:.4f}, rho={(k)/bp_base.size:.3f}")
    return sensor_order
...
def eval_candidates(metric, collector, metric_base, candidates, bp_base, saved_messages, status_nodes, current_obs, warm_iter, tol, damp, delta, k):
    best_score = -np.inf
    best_candidate = None
    ...
    for candidate in tqdm(candidates):
        ...
        overlap = metric(marginals, status_nodes=status_nodes, delta=delta)
        score = metric(marginals, status_nodes=status_nodes, delta=delta) - metric_base  # gain over current sensor set

        # Log to the collector buffer (not committed yet)
        collector.log_candidate(candidate=candidate, gain=score, overlap=overlap)
