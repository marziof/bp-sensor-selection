...
def full_sim(params, Gfixed = False, results_df = None, save=True):
    ...
    for method in params["methods"]:
        ...
        elif method in ["greedyOV", "greedyMOV", "greedyWarmStartOV", "greedyWarmStartMOV", "bp_it_entropy"]:
        ...
        elif method in ["greedySubsetOV", "greedySubsetMOV", "greedySampleReplaceOV", "greedySampleReplaceMOV", "MCMCsubsetOV", "MCMCsubsetMOV"]:
    ...
    sim_name = f"{methods_str}_N{params['N']}_d{params['d']}_T{params['T_max']}_Nsim{params['Nsim']}"
    results_df.to_csv(f"results/{sim_name}_plainMOV.csv", index=False)
    print(f"Results saved to results/{sim_name}_plainMOV.csv")
    sensors_df.to_csv(f"results/sensors_{sim_name}_plainMOV.csv", index=False)
    return results_df, sensors_df


import os

def save_results(results_df, sensors_df, params, suffix="checkpoint"):
    os.makedirs("results/checkpoints", exist_ok=True)
    methods_str = "_".join(params["methods"])
    sim_name = f"{methods_str}_N{params['N']}_d{params['d']}_T{params['T_max']}_Nsim{params['Nsim']}"
    
    results_path = f"results/checkpoints/{sim_name}_{suffix}.csv"
    results_df.to_csv(results_path, index=False)
    
    if sensors_df is not None:
        sensors_path = f"results/checkpoints/sensors_{sim_name}_{suffix}.csv"
...
def greedy_bp_sim_pipeline(param_list, Nsim, T_max, N, d, results_df, method, kind="rrg", Gfixed=False, sensors_df=None):
    print(f"Running greedy_bp_sim_pipeline with method={method}...")
    rho_list = sorted(set([rho for _, _, rho in param_list])) # get unique rho values from param_list
    for delta, lam, rho in tqdm(param_list, desc="Simulations"):
        if Gfixed:
            G, contacts, s0 = gen_graph_sim(N, d, lam=lam, T_max=T_max, delta=delta, kind=kind)
        for sim in range(Nsim):
            ...
            gt = s0 if method == "greedyOV" or method == "greedyWarmStartOV" else None
            if method in ["greedyOV", "greedyMOV"]:
                bp_fg, full_obs, selected_nodes_history, ov_history = run_bp_greedy(
                    [], rho_max=rho_list[-1], N=N, T=T_max,contacts=contacts, delta=delta,
                    status_nodes=status_nodes, max_iter=50, tol=1e-6, damp=0.5, gt=gt, m = int(0.1 * N)
                )
            elif method == "greedyWarmStartOV" or method == "greedyWarmStartMOV": # warm start version
                print("Running warm-start greedy...")
                # bp_fg, full_obs, selected_nodes_history, ov_history = run_bp_greedy_warm_start(
                #     [], rho_max=rho_list[-1], N=N, T=T_max, contacts=contacts, delta=delta,
                #     status_nodes=status_nodes, max_iter=100, warm_iter=10, tol=1e-6, damp=0.1, gt=gt, m=int(0.3 * N)
                # )
                bp_fg, full_obs, selected_nodes_history, ov_history, mov_history, ov_mov_tracker = run_bp_greedy_warm_start(
                    [],
                    rho_max=rho_list[-1],
                    N=N,
                    T=T_max,
                    ...
                    max_iter=100,    # full convergence from scratch
                    warm_iter=50,    # increased from 20 — needs ~9 iters to propagate across graph + convergence
                    tol=1e-4,        # slightly relaxed — don't need machine precision for ranking candidates
                    damp=0.2,        # reduced from 0.5 — faster convergence since we're already near fixed point
                    gt=gt, #status_nodes[0],  # use OV for now to diagnose, switch to None for MOV
                    print_progress=True,
                    m=int(0.1*N)   # 50 candidates per step
                )
            elif method == "bp_it_entropy":
                print("Running greedy by BP entropy...")
                bp_fg, full_obs, selected_nodes_history, ov_history, mov_history, entropy_history = run_bp_greedy_entropy(
                    [], rho_max=rho_list[-1], G=G, N=N, T=T_max, contacts=contacts, delta=delta,
                    status_nodes=status_nodes, max_iter=100, tol=1e-6, damp=0.5, gt=status_nodes[0], print_progress=True
                )
            else:
            ...
            # save check point
            save_results(results_df, sensors_df, params={"methods": [method], "N": N, "d": d, "T_max": T_max, "Nsim": Nsim}, suffix=f"checkpoint_tadapt_sim{sim}")



###---------------------- BO: Greedy sensor subset selection ----------------------###

def greedy_subset_bp_sim_pipeline(
    lam,
    delta,
    rho_list,
    Nsim,
    ...
    # method flags
    # ------------------------------------------------------------
    gt = method in ["greedySubsetOV", "MCMCsubsetOV"]
    sampleReplace = method in ["greedySampleReplaceOV", "greedySampleReplaceMOV"]
    mcmc = method in ["MCMCsubsetOV", "MCMCsubsetMOV"]

    ov_bayes_list = []

    # ------------------------------------------------------------
    # graph generation (fixed or per sim)
    # ------------------------------------------------------------
    if Gfixed:
        G, contacts, s0 = gen_graph_sim(N, d, lam=lam, T_max=T_max, delta=delta, kind=kind)
    ...
    for rho in rho_list:
        ...
        for sim in tqdm(range(Nsim)):
            ...
            if mcmc:
                ...
                gt_flag = (method == "MCMCsubsetOV")

                best_subset, best_reward, all_overlaps, bp_fg = sampleReplaceSubset_MCMC(
                    N=N,
                    T=T_max,
                    contacts=contacts,
                    delta=delta,
            ...
            # ------------------------------------------------------------
            bp_fg_rnd = fg.FactorGraph(N, T_max, contacts, [], delta)
            bp_fg_rnd.update(maxit=20, tol=1e-6, damp=0.5)
            ...
            # METRICS
            # ------------------------------------------------------------
            measures, x_est2 = compute_measures(marg, s0, delta, status_nodes, x_rnd, Mt_rnd)
            rank = compute_rank(marg, s0)
            precision, recall = compute_precision_recall(x_est, s0)
            f1 = compute_f1(precision, recall)
        ...
        if print_progress:
            print(f"\nrho={rho} OV mean:", np.mean(ov_bayes_list))


def greedy_subset_bp_sim_pipeline_old(lam, delta, rho_list, Nsim, T_max, N, d, results_df, method, kind="rrg", Gfixed = False, print_progress=False, sensors_df=None):
